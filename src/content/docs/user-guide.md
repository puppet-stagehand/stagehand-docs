...
means this tracking lives in the console's own database, not just in memory, so if the console itself gets restarted partway through a job, that job isn't
...
mistake, you can cancel it while it's running. This isn't just a status flip, the console actually stops the underlying comparison process, the same way
...
pull their value from a lookup function rather than a plain repository file, the console never executes that kind of function itself (running
