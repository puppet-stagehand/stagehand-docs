...
SSH remains the OpenVox execution path. Verify host keys through a trusted channel before the first connection, record the expected fingerprints in an organization-controlled location, and investigate unexpected key changes. Do not disable strict host verification to bypass a mismatch.
